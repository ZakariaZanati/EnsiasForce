...
export class CoursesService {
  ...
  createCourse(course : FormData){
    return this.http.post(this.SERVER_URL, course).subscribe(data => {
      console.log(data);
...
getCourse(id : number):Observable<CourseModel>{
  return this.http.get<CourseModel>(this.SERVER_URL + id)
}
...
getCoursesPage(pageNumber : number,pageSize : number,name : string,category : string,date : Date):Observable<CoursesResponse>{
  ...
  return this.http.get<CoursesResponse>(this.SERVER_URL, {params:params});
}
}
